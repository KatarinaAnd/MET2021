...
# Hvorfor vil den ikke lese inn denne sånn som på 2.3??
...
def extractdata(frostcfg,station,stmd,output):
    ...
    ds_station = ds_station.assign_coords({'lat': ds_station.latitude.values, 'lon': ds_station.longitude.values})
    ...
    # Den vil ikke legge til keywords :(
    whichhaskeywords = []
    for item in ds_station.data_vars.keys():
        if item in avvars:
            varname = item
            try:
                ds.station[item].attrs['keywords'] = get_keywords(item)
                whichhaskeywords.append[str(item)]
            except:
                continue
            print(varname)
