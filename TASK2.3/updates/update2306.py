...
def get_performance_category(chosen_category='A'):
    preformance_dict = {
...
def extractdata(frostcfg,station,stmd,output):
    ...
    dt = pd.read_csv('frost_standardname.csv')
    rad_dict = pd.Series(dt.standard_name.values,index = dt.element_id).to_dict()
    ds_station = xr.Dataset.from_dataframe(df)
