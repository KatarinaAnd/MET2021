...
def extractdata(frostcfg,station,stmd,output):
    ...
    if 'validTo' in metadata['data'][0].keys():
        if datetime.strptime(args.startday,'%Y-%m-%d') > datetime.strptime(metadata['data'][0]['validTo'],'%Y-%m-%dT%H:%M:%S.%fZ'): 
            #
            #
            # her vil vi legge til at det skal fylles in NaN f.o.m. datoen stasjonen var i drift
            #
            #
    ...
    for item in variables['data']:
        # lage en if statement som sier at dersom det er mellomrom eller parantes i varnavn, så kjør denne pathen.. blabla
        varname = item['elementId']
        #print(varname)
        #ds_station[varname].attrs['standard_name'] = varname


        ds_station[varname].attrs['long_name'] = 'et_eller_annet'
        ds_station[varname].attrs['units'] = item['unit']
        #ds_station['ny_ide'] = ds_station[varname]
        #ds_station = ds_station.drop(['{}'.format(varname)])
        #rename varname
