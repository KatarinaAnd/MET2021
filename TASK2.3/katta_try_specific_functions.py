...
def extractdata(frostcfg,station,stmd,output):
    ...
    # Here i have extracted the elements in the configuration-file, and suggested
    # that these are the ones we want in our dataset
    # fixed 18.06
    elements = frostcfg['elements'][0].split(',')
    elements = [x.rstrip(', ').lower() for x in elements]
    df = df[elements].copy()
    ...
    #thing = variables['data'][0]['elementId']
    #print(thing)
    #sys.exit()
    for item in variables['data']:
        varname = item['elementId'].rstrip(')').replace('(','_').replace(' ','_').lower()
        ...
        try:
            ...
            ds_station[varname].attrs['standard_name'] = varname
            ds_station[varname].attrs['long_name'] = varname
            ds_station[varname].attrs['units'] = item['unit']
    #print(ds_station)
    print('why')
    #rstrip()
